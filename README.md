...
- Messages are encrypted in transit, but this was built for fun/personal
  use on a trusted home/school/work or LAN networks, not as a hardened, audited
  security product. (It is hard to crack but you don't need a super computer or anything
...
- **Join approval** — every new connection pops up on the admin's screen
  for Accept/Reject before the person can chat.
- **Encrypted messages** — everything sent between client and server is
  encrypted, so it isn't sitting in plain text on the network.
...
### Important
If you are just running the precompiled EXEs, then you don't need to install anything. Not Python or the libraries—just run it as is, and it is a portable app.
...
The code should automatically install the pip liberes if you run it the ferst time with internet.

Or if you want to do i manually open a terminal in the folder you just downloaded/cloned, then run:
```bash
pip cryptography tkinter
```
This installs:
...
6. Message @admin or /admin at the start of a message and it lands
   directly in the admin's private inbox, separate from normal chat.
...
4. Right-click-free simple controls: select a user and hit **Kick** to
   disconnect them, or ban by username/IP so they can't rejoin.
...
7. Messages sent to `@admin`/`/admin` by any client show up in the
   **Admin inbox** list at the top of the panel.
...
- **Join approval flow** — when a client connects, it sends its username
  to the server, then waits. The server shows the admin an Accept/Reject
  popup; only after acceptance does the client's chat window open, and
  the client is added to the "Lobby" room.
- **Encryption** — every message is transformed with a Caesar cipher,
  ...
  `cryptography` library, which is AES under the hood) before being sent
  over the socket. The receiving side reverses the exact same steps.
  Both sides use the same shared key baked into the code.
